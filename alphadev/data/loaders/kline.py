...
class KlineDataLoader(DataLoader):
    ...
    def _get_file_path(self, symbol: str, trade_date: date) -> Optional[Path]:
        ...
        filename = f"{symbol}-{self.interval}-{trade_date.strftime('%Y-%m-%d')}.parquet.gz"
    ...
    def load_date_range(
        ...
        for trade_date in dates:
            for symbol in symbols:
                ...
                if file_path is None:
                    print(f"Warning: File not found for {symbol} on {trade_date}: {file_path}")
                    continue
                try:
        ...
        if not dfs:
            return pd.DataFrame(columns=['timestamp', 'symbol'] + self.columns).set_index(['timestamp', 'symbol'])
